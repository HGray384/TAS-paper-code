scm.un <- function (data, centered = FALSE) 
{
  if (!is.matrix(data)) 
    data <- as.matrix(data)
  p <- nrow(data)
  ...
  if (!centered) {
    ...
    TraceSigmaSquaredHat <- (N - 1)/(N * (N - 2) * (N - 
                                                      3)) * ((N - 1) * (N - 2) * sum(SigmaSample^2) + 
                                                               (TraceSigmaHat)^2 - N * Q)
    ...
    Term2 <- 2 * (sum(Sum1 * rowSums(data^2)) - sum(Sum21 + 
                                                      Sum22))
    Term3 <- 4 * (sum(Sum1^2) - sum(Sum3) - Term2)
    Term2 <- Term2/N/(N - 1)/(N - 2)
    Term3 <- Term3/N/(N - 1)/(N - 2)/(N - 3)
    TraceDiagonalSigmaSquaredHat <- Term1 - 2 * Term2 + 
      Term3
    LambdaHat <- (TraceSigmaHat^2 + TraceSigmaSquaredHat - 
                    2 * TraceDiagonalSigmaSquaredHat)/(N * TraceSigmaSquaredHat + 
    ...
    for (i in 1:(N - 1)) {
      TraceSigmaSquaredHat <- sum(crossprod(data[, i], 
                                            data[, (i + 1):N])^2) + TraceSigmaSquaredHat
      TraceDiagonalSigmaSquaredHat <- sum((data[, i] * 
                                             data[, (i + 1):N])^2) + TraceDiagonalSigmaSquaredHat
    }
    ...
    TraceDiagonalSigmaSquaredHat <- 2 * TraceDiagonalSigmaSquaredHat/N/(N - 
                                                                          1)
    LambdaHat <- (TraceSigmaHat^2 + TraceSigmaSquaredHat - 
                    2 * TraceDiagonalSigmaSquaredHat)/((N + 1) * TraceSigmaSquaredHat + 
  ...
  if (LambdaHat < 1) {
    ...
    diag(SigmaHat) <- DiagonalSigmaSample
  }
